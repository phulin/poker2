...
- Tried disabling model warm-start; it was slower and changed CFR output too much, so it was reverted.
...
- 2026-05-29 failed experiments:
  ...
  - `P2_EXACT_BELIEF_BATCH4=1`: a generated batch-4 exact-belief linear-in kernel had one short positive run, but the longer confirmation was slower, 551.6 ms baseline vs 569.5 ms candidate; reverted.
  ...
  - `P2_ZERO_SUM_HANDS_UNROLLED=1`: unrolling model zero-sum hand accumulation had short benchmark positives, but the stronger split-checkpoint sparse solve fixture failed on broader public spots with flop boundary public uniform action-prob max diff about 0.239 and root action-prob max diff about 0.076; reverted.
