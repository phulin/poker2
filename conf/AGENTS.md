...
- `config_rebel_curriculum_postflop.yaml`: Full fixed-schedule postflop curriculum config that runs river, turn, flop, and preflop-handoff substeps in one ordered orchestrator pass with per-train-stage live root overrides, S_i policy warm-starts from S_{i+1}, and legacy pre-chance value augmentation disabled.
- `config_rebel_curriculum_turn.yaml`: Postflop curriculum config for value-only `distill_E_turn` from promoted `S_river`, then live random-turn `S_turn` training with policy initialized from `S_river`, `E_turn` closing leaves, and legacy pre-chance value augmentation disabled.
- `config_rebel_curriculum_flop.yaml`: Postflop curriculum config for value-only `distill_E_flop` from promoted `S_turn`, live random-flop `S_flop` training with policy initialized from `S_turn` and `E_flop` closing leaves, then value-only `distill_E_preflop`, with legacy pre-chance value augmentation disabled.
