#!/usr/bin/env python3
"""
Detailed comparison of vectorized vs non-vectorized GAE implementations.
...
def compute_gae_non_vectorized(
    ...
    print(f"Trajectory starts: {starts.tolist()}")
    print(f"Trajectory ends: {ends.tolist()}")

    for traj_idx, (s, e) in enumerate(zip(starts.tolist(), ends.tolist())):
        print(f"\n--- Trajectory {traj_idx}: indices {s} to {e-1} ---")
        ...
        print(f"  Dones segment: {dseg.tolist()}")
        ...
        for t in range(e - s - 1, -1, -1):
            if t == e - s - 1:
                adv[t] = deltas[t]
                print(f"  t={t}: LAST -> adv[{t}] = delta[{t}] = {adv[t].item():.6f}")
            else:
                adv[t] = deltas[t] + gamma * lam * adv[t + 1] * (
                    1.0 - dseg[t].to(deltas.dtype)
                )
                print(
                    f"  t={t}: adv[{t}] = delta[{t}] + {gamma * lam:.6f} * adv[{t+1}] * (1-done[{t}])"
                )
                print(
                    f"       = {deltas[t].item():.6f} + {gamma * lam:.6f} * {adv[t + 1].item():.6f} * {1.0 - dseg[t].to(deltas.dtype):.6f}"
                )
                print(f"       = {adv[t].item():.6f}")
        advantages[s:e] = adv
        print(f"  Final trajectory advantages: {adv.tolist()}")

    print(f"\nFinal non-vectorized advantages: {advantages.tolist()}")
    print(f"=== END NON-VECTORIZED GAE DEBUG ===\n")
    return advantages


def test_gae_comparison():
    """Test GAE comparison with multiple trajectories."""

    # Create a test case with 4-5 trajectories
    # Each trajectory ends with a single done state
    rewards = torch.tensor(
        [
            0.0,
            0.0,
    ...
    print("=" * 80)
    print("GAE COMPARISON: VECTORIZED vs NON-VECTORIZED")
    print("=" * 80)
    print(f"Total timesteps: {len(rewards)}")
    print(f"Number of trajectories: {dones.sum().item()}")
    print(
        f"Trajectory lengths: {torch.diff(torch.cat([torch.tensor([0]), torch.where(dones)[0] + 1])).tolist()}"
    )
    print("=" * 80)
