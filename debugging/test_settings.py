...
def test_training_settings():
    ...
    for config in configs:
        ...
        print(
            f"  LR: {config['lr']}, Batch: {config['batch_size']}, Grad Clip: {config['grad_clip']}, Value Coef: {config['value_coef']}"
        )
        ...
        for step in range(5):
            ...
            if step == 4:  # Last step
                ...
                if len(trainer.replay_buffer.trajectories) >= trainer.batch_size:
                    ...
                    trajectories = trainer.replay_buffer.sample_trajectories(
                        trainer.batch_size
                    )
        ...
        print(
            f"  Loss trend: {'decreasing' if len(losses) > 1 and losses[-1] < losses[0] else 'stable/increasing'}"
        )
        print(f"  Final reward: {rewards[-1]:.2f}")
        print(
            f"  Reward trend: {'improving' if len(rewards) > 1 and rewards[-1] > rewards[0] else 'stable/worsening'}"
        )
