...
def analyze_policy_diversity(trainer):
    ...
    if len(all_policies) > 1:
        ...
        action_names = ["fold", "check/call", "bet 1/2", "bet 3/4", "bet pot", "bet 1.5x", "bet 2x", "bet 3x", "all-in"]
...
def analyze_model_sensitivity(trainer):
    ...
    print(f"State 1 - Hole cards: {state1.players[0].hole_cards}, Value: {value1.item():.6f}")
    print(f"State 2 - Hole cards: {state2.players[0].hole_cards}, Value: {value2.item():.6f}")
...
def main():
    ...
    print("This script analyzes how the model's policy varies across different game states.")
    print()
