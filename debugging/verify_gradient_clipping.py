...
def verify_gradient_clipping():
    ...
    loss = torch.sum(logits ** 2) + torch.sum(values ** 2) * 1000  # High value loss to simulate our issue
    ...
    returned_norm = torch.nn.utils.clip_grad_norm_(trainer.model.parameters(), trainer.grad_clip)
    
    print(f"Returned norm from clip_grad_norm_: {returned_norm:.6f}")
    print(f"Expected: {total_norm_before:.6f}")
    print(f"Returned norm matches expected: {abs(returned_norm - total_norm_before) < 1e-6}")
    ...
    if total_norm_after < total_norm_before:
        ...
        print(f"Gradients reduced by factor: {total_norm_before / total_norm_after:.6f}")
    else:
    ...
    clipped_grad_norm = torch.nn.utils.clip_grad_norm_(trainer.model.parameters(), trainer.grad_clip)
    ...
    print(f"  returned_norm ({clipped_grad_norm:.6f}) vs total_grad_norm_before ({total_grad_norm_before:.6f})")
