...
def detailed_training_analysis():
    ...
    for i in range(4):
        ...
        print(f"  Trajectory {i+1}: {len(trajectory.transitions)} transitions, "
    ...
    for i, trajectory in enumerate(trajectories):
        ...
        for j, transition in enumerate(trajectory.transitions):
            ...
            with torch.no_grad():
                ...
                print(f"    Step {j+1}: reward={rewards[j]:.2f}, value={value.item():.6f}")
    ...
    print(f"  Advantages - min: {min(all_advantages):.6f}, max: {max(all_advantages):.6f}, "
          f"mean: {sum(all_advantages)/len(all_advantages):.6f}")
    print(f"  Returns - min: {min(all_returns):.6f}, max: {max(all_returns):.6f}, "
    ...
    original_norm = torch.nn.utils.clip_grad_norm_(trainer.model.parameters(), trainer.grad_clip)
