...
def simple_gradient_clipping_test():
    ...
    clipped_norm = torch.nn.utils.clip_grad_norm_(trainer.model.parameters(), trainer.grad_clip)
    ...
    for clip_val in [0.1, 0.5, 1.0, 5.0]:
        ...
        # Apply clipping
        clipped_norm = torch.nn.utils.clip_grad_norm_(trainer.model.parameters(), clip_val)
        
        print(f"Clip {clip_val}: {total_norm:.6f} -> {clipped_norm:.6f} (ratio: {clipped_norm/total_norm:.6f})")
