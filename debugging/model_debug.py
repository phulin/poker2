...
def debug_model_internals(trainer):
    ...
    with torch.no_grad():
        ...
        print(
            f"  Cards trunk output: {x_cards.shape}, norm: {torch.norm(x_cards).item():.6f}"
        )
        print(
            f"  Actions trunk output: {x_actions.shape}, norm: {torch.norm(x_actions).item():.6f}"
        )
        ...
        print(
            f"  Cards trunk - zero outputs: {(x_cards == 0).sum().item()}/{x_cards.numel()}"
        )
        print(
            f"  Actions trunk - zero outputs: {(x_actions == 0).sum().item()}/{x_actions.numel()}"
        )
        print(f"  Fusion - zero outputs: {(h == 0).sum().item()}/{h.numel()}")
        print(
            f"  Policy logits - zero outputs: {(logits == 0).sum().item()}/{logits.numel()}"
        )
