...
def analyze_hand(trainer):
    ...
    env = HUNLEnv(starting_stack=400, sb=1, bb=2, seed=123)  # Different seed for variety
    ...
    print(f"Player 0 stack: {state.players[0].stack}, Player 1 stack: {state.players[1].stack}")
    ...
    while not state.terminal and step_count < max_steps:
        ...
        action_names = ["fold", "check/call", "bet 1/2", "bet 3/4", "bet pot", "bet 1.5x", "bet 2x", "all-in"]
        ...
        print(f"Action confidence: {'High' if best_prob > 0.7 else 'Medium' if best_prob > 0.4 else 'Low'}")
