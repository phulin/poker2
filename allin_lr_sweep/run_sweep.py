...
TRAIN = (
    "outputs/allin_training_data_512k_s65536_b4096_bc64/"
    "manifest_players6_train200.json"
)
VAL = (
    "outputs/allin_training_data_512k_s65536_b4096_bc64/"
    "manifest_players6_val31.json"
)
LOG_DIR = ROOT / "allin_lr_sweep" / "logs"
CKPT_ROOT = ROOT / "allin_lr_sweep" / "checkpoints"
STEPS = 1000


CANDIDATES = [
    {
        "name": "lr015_cos500_r015",
        "learning_rate": 0.015,
    ...
    {
        "name": "lr010_cos2000_r03",
        "learning_rate": 0.010,
        "adamw_learning_rate": 0.016,
        "cosine_lr_decay_ratio": 0.030,
        "cosine_lr_decay_steps": 2000,
    },
    {
        "name": "lr008_cos4000_r05",
        "learning_rate": 0.008,
        "adamw_learning_rate": 0.0128,
        "cosine_lr_decay_ratio": 0.050,
...
TRAIN_RE = re.compile(
    r"\[(?P<step>\d+)/1000\] bs=512 mse=(?P<mse>[0-9.]+) "
    r"mae=(?P<mae>[0-9.]+) target=(?P<target>[0-9.]+)s "
    r"step=(?P<seconds>[0-9.]+)s"
)
EVAL_RE = re.compile(
    r"\[eval (?P<step>\d+)/1000\] mse=(?P<mse>[0-9.]+) "
...
def main() -> None:
    ...
    for candidate in CANDIDATES:
        ...
        cmd = [
            ...
            f"lr_decay={candidate.get('lr_decay', 'cosine')}",
