...
class CardsPlanesV1(Encoder):
    def encode_cards(self, game_state: Any, seat: int) -> Any:
        ...
        flop = (
            _cards_to_planes(game_state.board[:3])
            if len(game_state.board) >= 3
            else torch.zeros((4, 13), dtype=torch.float32)
        )
        turn = (
            _cards_to_planes(game_state.board[3:4])
            if len(game_state.board) >= 4
            else torch.zeros((4, 13), dtype=torch.float32)
        )
        river = (
            _cards_to_planes(game_state.board[4:5])
