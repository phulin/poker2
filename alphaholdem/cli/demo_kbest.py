...
def demonstrate_kbest():
    ...
    for step in range(10):
        ...
        if step % 3 == 0:
            ...
            if pool_stats['pool_size'] > 0:
                ...
                print(f"  - Pool ELO range: {pool_stats['min_elo']:.1f} - {pool_stats['max_elo']:.1f}")
        print()
    ...
    for opponent_key, result in eval_results['opponent_results'].items():
        print(f"  {opponent_key}: {result['win_rate']:.3f} (ELO: {result['opponent_elo']:.1f})")
    print()
...
def explain_kbest_concept():
    ...
    print("3. Diverse Sampling: Trains against different opponents to avoid overfitting")
    print("4. Dynamic Updates: Adds new snapshots when significant improvement occurs\n")
