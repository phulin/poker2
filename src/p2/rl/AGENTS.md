...
- `cfr_trainer.py`: ReBeL CFR supervised trainer, including split BetterFFN policy/value ownership, value-only distillation updates, PBSEnv-backed multiway setup, non-fused closing-leaf checkpoint routing, data-source state checkpointing, and replay-buffer sidecar save/load.
