...
- `better_feature_encoder.py`: BetterFFN policy and street-boundary value feature encoders for richer public-belief inputs, including compact 169-hand preflop encoders.
- `better_ffn.py`: Residual feed-forward Better models, including the legacy combined model, compact 169-hand preflop policy/value variants with dtype-aligned compact feature inputs, plus split `BetterPolicyFFN`, `BetterStreetValueFFN`, and `BetterSplitFFN` wrapper with static feature-base helpers, multiway policy blocker aggregation, and optional low-rank rank/suit board interactions.
