...
- `pregenerate_postflop_rebel.py`: Bounded postflop ReBeL solved-example dataset writer for HP sweeps/holdouts, reusing live CFR generation, tensor-only shard output with optional compressed float storage, sampler metadata, normalized root-street metadata, feature-encoder metadata, static CFR quality metadata, and closing-leaf/distillation-source checkpoint provenance.
