...
- `pregenerate_postflop_rebel.py`: Bounded postflop ReBeL solved-example dataset writer for HP sweeps/holdouts, reusing live CFR generation, tensor-only shard output with optional compressed float storage, sampler metadata, and closing-leaf checkpoint provenance.
