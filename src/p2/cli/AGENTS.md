...
- `train_rebel_curriculum.py`: Staged postflop ReBeL curriculum orchestrator that runs configured train and end-of-street value-only distill substeps with per-substep checkpoints, promotion, metadata, S_i policy warm-starts from promoted S_{i+1} checkpoints, and validated substep-aware resume.
