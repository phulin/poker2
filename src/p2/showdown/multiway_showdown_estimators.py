...
@dataclass
class BatchedAliasTupleRejectWorkspace:
    alias_prob: torch.Tensor
    alias_idx: torch.Tensor
    seed: torch.Tensor
    small_stack: torch.Tensor
...
@dataclass
class BatchedHierCDFTupleRejectWorkspace:
    local_cdf: torch.Tensor
    block_cdf: torch.Tensor
    seed: torch.Tensor
    accum: torch.Tensor
    equity: torch.Tensor
    stderr: torch.Tensor
...
@dataclass
class BatchedCDFTupleRejectWorkspace:
    cdf: torch.Tensor
    seed: torch.Tensor
    accum: torch.Tensor
    equity: torch.Tensor
...
@dataclass
class BatchedHierAliasCDFTupleRejectWorkspace:
    local_alias_prob: torch.Tensor
    local_alias_idx: torch.Tensor
    block_cdf: torch.Tensor
    small_stack: torch.Tensor
    large_stack: torch.Tensor
    counts: torch.Tensor
...
def exact_nway_ie_by_hand(
    ...
    *,
    ...
    numerator_by_hand = torch.zeros(players, NUM_HANDS, dtype=dtype, device=device)
    denominator_by_hand = torch.zeros(players, NUM_HANDS, dtype=dtype, device=device)
    equity_by_hand = torch.zeros(players, NUM_HANDS, dtype=dtype, device=device)
    for hero in range(players):
        ...
        equities.append(aggregate)
        numerator_by_hand[hero] = numerator
        denominator_by_hand[hero] = denominator
        equity_by_hand[hero] = safe_divide_by_hand(numerator, denominator)

    allowed = board_allowed_hands(prepared.board)[0].to(dtype)
    numerator_by_hand = numerator_by_hand * allowed[None, :]
    denominator_by_hand = denominator_by_hand * allowed[None, :]
    equity_by_hand = equity_by_hand * allowed[None, :]
    aggregate_equity = torch.stack(equities).to(torch.float32)[None, :]
    if device.type == "cuda":
        torch.cuda.synchronize(device)
...
def exact_nway_ie(
    ...
    *,
    ...
    result = exact_nway_ie_by_hand(
        prepared,
        pattern_chunk=pattern_chunk,
        dtype=dtype,
        chunked_f32=chunked_f32,
    )
...
def make_batched_active_belief_workspace(
    board: PreparedFastSISBoard,
    *,
    batch_size: int,
    players: int,
    ...
    return PreparedBatchedFastSISBelief(
        board=board,
        beliefs=torch.empty(rows, active_hands, dtype=torch.float32, device=device),
        card_mass=torch.empty(0, dtype=torch.float32, device=device),
        pair_mass=torch.empty(0, dtype=torch.float16, device=device),
        batch_size=batch_size,
        players=players,
        setup_seconds=0.0,
    )
...
def prepare_batched_active_belief_triton_into(
    ...
    *,
    ...
    if beliefs.device.type != "cuda":
        raise ValueError("batched active belief prep requires CUDA tensors")
    if workspace.card_mass.numel() or workspace.pair_mass.numel():
        raise ValueError("active-only belief prep expects empty card and pair mass tensors")
    start = time.perf_counter()
    active_hands = workspace.beliefs.shape[1]
    block_h = triton.next_power_of_2(active_hands)
    _prepare_batched_active_belief_kernel[(workspace.beliefs.shape[0],)](
        beliefs.contiguous(),
        workspace.board.active_ids,
        workspace.beliefs,
        active_hands=active_hands,
...
if triton is not None:
    ...
    @triton.jit
    def _prepare_batched_active_belief_kernel(
        full_beliefs_ptr,
        active_ids_ptr,
        active_beliefs_ptr,
        active_hands: tl.constexpr,
    ...
    @triton.jit
    def _alias_tuple_reject_accumulate_batched_kernel(
        alias_prob_ptr,
        alias_idx_ptr,
        hand_masks_ptr,
        hand_ranks_ptr,
        ...
        for attempt in tl.static_range(0, TUPLE_TRIES):
            ...
            valid = active
            used = tl.full((BLOCK_S,), 0, tl.int64)
            th0 = tl.full((BLOCK_S,), 0, tl.int32)
            th1 = tl.full((BLOCK_S,), 0, tl.int32)
            th2 = tl.full((BLOCK_S,), 0, tl.int32)
            th3 = tl.full((BLOCK_S,), 0, tl.int32)
            th4 = tl.full((BLOCK_S,), 0, tl.int32)
            th5 = tl.full((BLOCK_S,), 0, tl.int32)
            ...
            for player in tl.static_range(0, 6):
                if player < PLAYERS:
                    ...
                    u_pick = x1.to(tl.float32) * 2.3283064365386963e-10
                    column = tl.minimum((u_col * active_hands).to(tl.int32), active_hands - 1)
                    sampling = active & valid
                    cutoff = tl.load(alias_prob_ptr + row_offset + column, mask=sampling, other=1.0)
                    alias = tl.load(alias_idx_ptr + row_offset + column, mask=sampling, other=0)
                    candidate = tl.where(u_pick < cutoff, column, alias)
            ...
            take = active & valid
            alive = alive | take
            r0 = tl.where(take, tl.load(hand_ranks_ptr + th0, mask=take, other=-1), r0)
            r1 = tl.where(take, tl.load(hand_ranks_ptr + th1, mask=take, other=-1), r1)
            r2 = tl.where(take, tl.load(hand_ranks_ptr + th2, mask=take, other=-1), r2)
            r3 = tl.where(take, tl.load(hand_ranks_ptr + th3, mask=take, other=-1), r3)
            r4 = tl.where(take, tl.load(hand_ranks_ptr + th4, mask=take, other=-1), r4)
            r5 = tl.where(take, tl.load(hand_ranks_ptr + th5, mask=take, other=-1), r5)
        ...
        inv_tie = 1.0 / tl.maximum(tie_count, 1.0)
        weight = tl.where(alive, 1.0, 0.0)
        fail_count = (in_range & ~alive).to(tl.float32)
        accum_base = batch * (3 + 2 * PLAYERS)

        tl.atomic_add(accum_ptr + accum_base + 0, tl.sum(weight, axis=0), sem="relaxed")
        tl.atomic_add(accum_ptr + accum_base + 1, tl.sum(weight, axis=0), sem="relaxed")
        tl.atomic_add(accum_ptr + accum_base + 2, tl.sum(fail_count, axis=0), sem="relaxed")
        share0 = tl.where(alive & w0, inv_tie, 0.0)
        share1 = tl.where(alive & w1, inv_tie, 0.0)
        tl.atomic_add(accum_ptr + accum_base + 3, tl.sum(share0, axis=0), sem="relaxed")
        tl.atomic_add(accum_ptr + accum_base + 4, tl.sum(share1, axis=0), sem="relaxed")
        tl.atomic_add(accum_ptr + accum_base + 3 + PLAYERS, tl.sum(share0 * share0, axis=0), sem="relaxed")
        tl.atomic_add(accum_ptr + accum_base + 4 + PLAYERS, tl.sum(share1 * share1, axis=0), sem="relaxed")
        if PLAYERS > 2:
            share2 = tl.where(alive & w2, inv_tie, 0.0)
            tl.atomic_add(accum_ptr + accum_base + 5, tl.sum(share2, axis=0), sem="relaxed")
            tl.atomic_add(accum_ptr + accum_base + 5 + PLAYERS, tl.sum(share2 * share2, axis=0), sem="relaxed")
        if PLAYERS > 3:
            share3 = tl.where(alive & w3, inv_tie, 0.0)
            tl.atomic_add(accum_ptr + accum_base + 6, tl.sum(share3, axis=0), sem="relaxed")
            tl.atomic_add(accum_ptr + accum_base + 6 + PLAYERS, tl.sum(share3 * share3, axis=0), sem="relaxed")
        if PLAYERS > 4:
            share4 = tl.where(alive & w4, inv_tie, 0.0)
            tl.atomic_add(accum_ptr + accum_base + 7, tl.sum(share4, axis=0), sem="relaxed")
            tl.atomic_add(accum_ptr + accum_base + 7 + PLAYERS, tl.sum(share4 * share4, axis=0), sem="relaxed")
        if PLAYERS > 5:
            share5 = tl.where(alive & w5, inv_tie, 0.0)
            tl.atomic_add(accum_ptr + accum_base + 8, tl.sum(share5, axis=0), sem="relaxed")
            tl.atomic_add(accum_ptr + accum_base + 8 + PLAYERS, tl.sum(share5 * share5, axis=0), sem="relaxed")

    @triton.jit
    def _build_hier_cdf_local_kernel(
        beliefs_ptr,
        local_cdf_ptr,
        block_sums_ptr,
        active_hands: tl.constexpr,
        NUM_BLOCKS: tl.constexpr,
        BLOCK_CDF: tl.constexpr,
    ):
        row = tl.program_id(0)
        block = tl.program_id(1)
        offs = tl.arange(0, BLOCK_CDF)
        hand = block * BLOCK_CDF + offs
        active = hand < active_hands
        values = tl.load(
            beliefs_ptr + row * active_hands + hand,
            mask=active,
            other=0.0,
        )
        prefix = tl.cumsum(values, 0)
        out_base = row * NUM_BLOCKS * BLOCK_CDF + block * BLOCK_CDF
        tl.store(local_cdf_ptr + out_base + offs, prefix, mask=offs < BLOCK_CDF)
        tl.store(block_sums_ptr + row * NUM_BLOCKS + block, tl.sum(values, axis=0))

    @triton.jit
    def _build_hier_cdf_block_kernel(
        block_sums_ptr,
        block_cdf_ptr,
        NUM_BLOCKS: tl.constexpr,
        ...
        active = offs < NUM_BLOCKS
        values = tl.load(block_sums_ptr + row * NUM_BLOCKS + offs, mask=active, other=0.0)
        prefix = tl.cumsum(values, 0)
        tl.store(block_cdf_ptr + row * NUM_BLOCKS + offs, prefix, mask=active)

    @triton.jit
    def _build_direct_cdf_kernel(
        beliefs_ptr,
        cdf_ptr,
        active_hands: tl.constexpr,
        BLOCK_H: tl.constexpr,
        ...
        offs = tl.arange(0, BLOCK_H)
        active = offs < active_hands
        values = tl.load(beliefs_ptr + row * active_hands + offs, mask=active, other=0.0)
        prefix = tl.cumsum(values, 0)
        tl.store(cdf_ptr + row * active_hands + offs, prefix, mask=active)

    @triton.jit
    def _build_hier_alias_cdf_local_partition_kernel(
        beliefs_ptr,
        local_alias_prob_ptr,
        local_alias_idx_ptr,
        block_sums_ptr,
        small_stack_ptr,
        large_stack_ptr,
        ...
        scaled = values * BLOCK_CDF / tl.maximum(block_sum, 1.0e-30)
        local_block = row * NUM_BLOCKS + block
        local_base = local_block * BLOCK_CDF
        tl.store(local_alias_prob_ptr + local_base + offs, scaled)
        tl.store(local_alias_idx_ptr + local_base + offs, offs)
        ...
        small_count = tl.sum(small_mask.to(tl.int32), axis=0)
        large_count = tl.sum(large_mask.to(tl.int32), axis=0)
        tl.store(small_stack_ptr + local_base + small_rank, offs, mask=small_mask)
        tl.store(large_stack_ptr + local_base + large_rank, offs, mask=large_mask)
        tl.store(counts_ptr + local_block * 2, small_count)
        tl.store(counts_ptr + local_block * 2 + 1, large_count)
        tl.store(block_sums_ptr + row * NUM_BLOCKS + block, block_sum)

    @triton.jit
    def _hier_alias_cdf_local_resolve_kernel(
        local_alias_prob_ptr,
        local_alias_idx_ptr,
        small_stack_ptr,
        large_stack_ptr,
        ...
        local_base = local_block * BLOCK_CDF
        small_count = tl.load(counts_ptr + local_block * 2)
        large_count = tl.load(counts_ptr + local_block * 2 + 1)

        while (small_count > 0) & (large_count > 0):
            small_count -= 1
            small_idx = tl.load(small_stack_ptr + local_base + small_count)
            large_idx = tl.load(large_stack_ptr + local_base + large_count - 1)
            small_value = tl.load(local_alias_prob_ptr + local_base + small_idx)
            large_value = tl.load(local_alias_prob_ptr + local_base + large_idx)
            tl.store(local_alias_idx_ptr + local_base + small_idx, large_idx)
            new_large_value = large_value + small_value - 1.0
            tl.store(local_alias_prob_ptr + local_base + large_idx, new_large_value)
            if new_large_value < 1.0:
                large_count -= 1
                tl.store(small_stack_ptr + local_base + small_count, large_idx)
                small_count += 1

        while large_count > 0:
            large_count -= 1
            idx = tl.load(large_stack_ptr + local_base + large_count)
            tl.store(local_alias_prob_ptr + local_base + idx, 1.0)
            tl.store(local_alias_idx_ptr + local_base + idx, idx)

        while small_count > 0:
            small_count -= 1
            idx = tl.load(small_stack_ptr + local_base + small_count)
            tl.store(local_alias_prob_ptr + local_base + idx, 1.0)
            tl.store(local_alias_idx_ptr + local_base + idx, idx)

    @triton.jit
    def _direct_cdf_tuple_reject_accumulate_batched_kernel(
        cdf_ptr,
        hand_masks_ptr,
        hand_ranks_ptr,
        accum_ptr,
        ...
        for attempt in tl.static_range(0, TUPLE_TRIES):
            ...
            for player in tl.static_range(0, 6):
                if player < PLAYERS:
                    ...
                    x0 = _pcg_hash_u32(random_offset.to(tl.uint32) + seed_mix)
                    u = x0.to(tl.float32) * 2.3283064365386963e-10
                    total = tl.load(cdf_ptr + row_offset + active_hands - 1)
                    target = u * total

                    lo = tl.full((BLOCK_S,), 0, tl.int32)
                    hi = tl.full((BLOCK_S,), active_hands, tl.int32)
                    for _ in tl.static_range(0, CDF_STEPS):
                        mid = (lo + hi) // 2
                        mid_cdf = tl.load(
                            cdf_ptr + row_offset + mid,
                            mask=mid < active_hands,
                            other=float("inf"),
                        )
                        go_left = mid_cdf > target
    ...
    @triton.jit
    def _hier_cdf_tuple_reject_accumulate_batched_kernel(
        local_cdf_ptr,
        block_cdf_ptr,
        hand_masks_ptr,
        hand_ranks_ptr,
        ...
        for attempt in tl.static_range(0, TUPLE_TRIES):
            ...
            for player in tl.static_range(0, 6):
                if player < PLAYERS:
                    ...
                    x0 = _pcg_hash_u32(random_offset.to(tl.uint32) + seed_mix)
                    u = x0.to(tl.float32) * 2.3283064365386963e-10
                    total = tl.load(block_cdf_ptr + row * NUM_BLOCKS + NUM_BLOCKS - 1)
                    target = u * total

                    lo = tl.full((BLOCK_S,), 0, tl.int32)
                    hi = tl.full((BLOCK_S,), NUM_BLOCKS, tl.int32)
                    for _ in tl.static_range(0, BLOCK_STEPS):
                        mid = (lo + hi) // 2
                        mid_cdf = tl.load(
                            block_cdf_ptr + row * NUM_BLOCKS + mid,
                            mask=mid < NUM_BLOCKS,
                            other=float("inf"),
                        )
                        go_left = mid_cdf > target
                        hi = tl.where(go_left, mid, hi)
                        lo = tl.where(go_left, lo, mid + 1)
                    block = tl.minimum(lo, NUM_BLOCKS - 1)
                    prev_block_cdf = tl.load(
                        block_cdf_ptr + row * NUM_BLOCKS + block - 1,
                        mask=block > 0,
                        other=0.0,
                    )
                    local_target = target - prev_block_cdf

                    local_lo = tl.full((BLOCK_S,), 0, tl.int32)
                    local_hi = tl.full((BLOCK_S,), BLOCK_CDF, tl.int32)
                    local_base = row * NUM_BLOCKS * BLOCK_CDF + block * BLOCK_CDF
                    for _ in tl.static_range(0, LOCAL_STEPS):
                        mid = (local_lo + local_hi) // 2
                        local_value = tl.load(
                            local_cdf_ptr + local_base + mid,
                            mask=mid < BLOCK_CDF,
                            other=float("inf"),
                        )
                        go_left = local_value > local_target
                        local_hi = tl.where(go_left, mid, local_hi)
                        local_lo = tl.where(go_left, local_lo, mid + 1)
                    candidate = tl.minimum(block * BLOCK_CDF + local_lo, active_hands - 1)
...
def make_batched_alias_tuple_reject_workspace(
    prepared: PreparedBatchedFastSISBelief,
    *,
    sample_count: int,
    tuple_tries: int = 3,
    ...
    device = prepared.beliefs.device
    rows, active_hands = prepared.beliefs.shape
    return BatchedAliasTupleRejectWorkspace(
        alias_prob=torch.empty(rows, active_hands, dtype=torch.float32, device=device),
        alias_idx=torch.empty(rows, active_hands, dtype=torch.int16, device=device),
        seed=torch.empty((), dtype=torch.int64, device=device),
        small_stack=torch.empty(rows, active_hands, dtype=torch.int32, device=device),
...
def make_batched_hier_cdf_tuple_reject_workspace(
    prepared: PreparedBatchedFastSISBelief,
    *,
    sample_count: int,
    tuple_tries: int = 3,
    block_size: int = 32,
) -> BatchedHierCDFTupleRejectWorkspace:
    start = time.perf_counter()
    device = prepared.beliefs.device
    rows, active_hands = prepared.beliefs.shape
    num_blocks = triton.cdiv(active_hands, block_size)
    return BatchedHierCDFTupleRejectWorkspace(
        local_cdf=torch.empty(
            rows,
            num_blocks * block_size,
            dtype=torch.float32,
            device=device,
        ),
        block_cdf=torch.empty(rows, num_blocks, dtype=torch.float32, device=device),
        seed=torch.empty((), dtype=torch.int64, device=device),
        accum=torch.empty(
            prepared.batch_size,
            3 + 2 * prepared.players,
...
def make_batched_cdf_tuple_reject_workspace(
    prepared: PreparedBatchedFastSISBelief,
    *,
    sample_count: int,
    tuple_tries: int = 3,
) -> BatchedCDFTupleRejectWorkspace:
    start = time.perf_counter()
    device = prepared.beliefs.device
    rows, active_hands = prepared.beliefs.shape
    return BatchedCDFTupleRejectWorkspace(
        cdf=torch.empty(rows, active_hands, dtype=torch.float32, device=device),
        seed=torch.empty((), dtype=torch.int64, device=device),
        accum=torch.empty(
            prepared.batch_size,
            3 + 2 * prepared.players,
...
def build_batched_alias_tables_triton_into(
    prepared: PreparedBatchedFastSISBelief,
    workspace: BatchedAliasRejectSISWorkspace | BatchedAliasTupleRejectWorkspace,
    *,
...
def build_batched_hier_cdf_triton_into(
    prepared: PreparedBatchedFastSISBelief,
    workspace: BatchedHierCDFTupleRejectWorkspace,
    *,
    build_warps: int = 1,
    synchronize: bool = False,
) -> None:
    if triton is None:
        raise RuntimeError("triton is not available")
    rows, active_hands = prepared.beliefs.shape
    num_blocks = triton.cdiv(active_hands, workspace.block_size)
    if workspace.block_cdf.shape != (rows, num_blocks):
        raise ValueError("hierarchical CDF workspace shape does not match prepared batch")
    block_b = triton.next_power_of_2(num_blocks)
    _build_hier_cdf_local_kernel[(rows, num_blocks)](
        prepared.beliefs,
        workspace.local_cdf,
        workspace.block_cdf,
        active_hands=active_hands,
        NUM_BLOCKS=num_blocks,
        BLOCK_CDF=workspace.block_size,
        num_warps=build_warps,
    )
    _build_hier_cdf_block_kernel[(rows,)](
        workspace.block_cdf,
        workspace.block_cdf,
        NUM_BLOCKS=num_blocks,
        BLOCK_B=block_b,
...
def build_batched_direct_cdf_triton_into(
    prepared: PreparedBatchedFastSISBelief,
    workspace: BatchedCDFTupleRejectWorkspace,
    *,
    build_warps: int = 8,
    synchronize: bool = False,
) -> None:
    if triton is None:
        raise RuntimeError("triton is not available")
    if workspace.cdf.shape != prepared.beliefs.shape:
        raise ValueError("direct CDF workspace shape does not match prepared batch")
    active_hands = prepared.beliefs.shape[1]
    block_h = triton.next_power_of_2(active_hands)
    _build_direct_cdf_kernel[(prepared.beliefs.shape[0],)](
        prepared.beliefs,
        workspace.cdf,
        active_hands=active_hands,
        BLOCK_H=block_h,
...
def alias_triton_tuple_reject_batched_fixed_into(
    prepared: PreparedBatchedFastSISBelief,
    workspace: BatchedAliasTupleRejectWorkspace,
    *,
    block_s: int = 256,
    seed: int,
) -> None:
    ...
    workspace.seed.fill_(seed)
    workspace.accum.zero_()
    grid = (prepared.batch_size, triton.cdiv(workspace.sample_count, block_s))
    _alias_tuple_reject_accumulate_batched_kernel[grid](
        workspace.alias_prob,
        workspace.alias_idx,
        prepared.board.hand_masks,
        prepared.board.hand_ranks,
...
def hier_cdf_tuple_reject_batched_fixed_into(
    prepared: PreparedBatchedFastSISBelief,
    workspace: BatchedHierCDFTupleRejectWorkspace,
    *,
    block_s: int = 256,
    ...
    workspace.accum.zero_()
    grid = (prepared.batch_size, triton.cdiv(workspace.sample_count, block_s))
    _hier_cdf_tuple_reject_accumulate_batched_kernel[grid](
        workspace.local_cdf,
        workspace.block_cdf,
        prepared.board.hand_masks,
        prepared.board.hand_ranks,
        ...
        NUM_BLOCKS=num_blocks,
        BLOCK_CDF=workspace.block_size,
        BLOCK_STEPS=math.ceil(math.log2(num_blocks)),
        LOCAL_STEPS=math.ceil(math.log2(workspace.block_size)),
        BLOCK_S=block_s,
        num_warps=sample_warps,
    )
    _finalize_batched_accum_kernel[(prepared.batch_size,)](
        workspace.accum,
        workspace.equity,
...
def direct_cdf_tuple_reject_batched_fixed_into(
    prepared: PreparedBatchedFastSISBelief,
    workspace: BatchedCDFTupleRejectWorkspace,
    *,
    block_s: int = 256,
    ...
    workspace.accum.zero_()
    grid = (prepared.batch_size, triton.cdiv(workspace.sample_count, block_s))
    _direct_cdf_tuple_reject_accumulate_batched_kernel[grid](
        workspace.cdf,
        prepared.board.hand_masks,
        prepared.board.hand_ranks,
        workspace.accum,
        ...
        PLAYERS=prepared.players,
        TUPLE_TRIES=workspace.tuple_tries,
        CDF_STEPS=math.ceil(math.log2(active_hands)),
        BLOCK_S=block_s,
        num_warps=sample_warps,
    )
    _finalize_batched_accum_kernel[(prepared.batch_size,)](
...
def run_batched_belief_benchmark(
    ...
    if args.batch_belief_sampler in ("tuple-reject", "hier-cdf", "direct-cdf"):
        belief_workspace = make_batched_active_belief_workspace(
            fast_board,
            batch_size=batch_size,
            players=args.players,
        )
        prepare_beliefs_into = prepare_batched_active_belief_triton_into
    else:
        ...
        prepare_beliefs_into = prepare_batched_fast_sis_belief_no_pair_triton_into
    prepare_beliefs_into(
        belief_workspace,
        full_beliefs,
        normalize=False,
    )
    ...
    if args.batch_belief_sampler == "alias":
    ...
    elif args.batch_belief_sampler == "tuple-reject":
        reject_workspace = make_batched_alias_tuple_reject_workspace(
            belief_workspace,
            sample_count=sample_count,
            tuple_tries=args.batch_belief_tuple_tries,
        )
        if args.batch_belief_reuse_alias:
            build_batched_alias_tables_triton_into(
                belief_workspace,
        ...
        alias_setup_seconds = reject_workspace.setup_seconds
    elif args.batch_belief_sampler == "hier-cdf":
        reject_workspace = make_batched_hier_cdf_tuple_reject_workspace(
            belief_workspace,
            sample_count=sample_count,
            tuple_tries=args.batch_belief_tuple_tries,
            block_size=args.batch_belief_hier_cdf_block,
        )
        if args.batch_belief_reuse_alias:
            build_batched_hier_cdf_triton_into(
                belief_workspace,
                reject_workspace,
                build_warps=args.batch_belief_cdf_build_warps,
                synchronize=False,
            )
        alias_setup_seconds = reject_workspace.setup_seconds
    elif args.batch_belief_sampler == "direct-cdf":
        reject_workspace = make_batched_cdf_tuple_reject_workspace(
            belief_workspace,
            sample_count=sample_count,
            tuple_tries=args.batch_belief_tuple_tries,
        )
        if args.batch_belief_reuse_alias:
            build_batched_direct_cdf_triton_into(
                belief_workspace,
                reject_workspace,
                build_warps=args.batch_belief_cdf_build_warps,
                synchronize=False,
    ...
    for _ in range(args.batch_belief_warmup_iterations):
        ...
        if args.batch_belief_sampler == "multinomial":
        ...
        elif args.batch_belief_sampler == "alias":
            ...
            hier_cdf_tuple_reject_batched_fixed_into(
                belief_workspace,
                reject_workspace,
                block_s=block_s,
                seed=args.seed,
                sample_warps=args.batch_belief_cdf_sample_warps,
            )
        else:
            if not args.batch_belief_reuse_alias:
                build_batched_direct_cdf_triton_into(
            ...
            direct_cdf_tuple_reject_batched_fixed_into(
                belief_workspace,
                reject_workspace,
                block_s=block_s,
                seed=args.seed,
    ...
    if args.batch_belief_sampler in ("tuple-reject", "hier-cdf", "direct-cdf"):
        proposals = samples * args.players * args.batch_belief_tuple_tries
    else:
    ...
    print(
        ...
        f"tuple_tries={args.batch_belief_tuple_tries} "
        f"hier_cdf_block={args.batch_belief_hier_cdf_block} "
        f"cdf_build_warps={args.batch_belief_cdf_build_warps} "
        f"cdf_sample_warps={args.batch_belief_cdf_sample_warps} "
        ...
        f"alias_build_in_loop={args.batch_belief_sampler in ('alias', 'tuple-reject', 'hier-cdf', 'direct-cdf') and not args.batch_belief_reuse_alias} "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=("alias", "multinomial", "tuple-reject", "hier-cdf", "direct-cdf"),
        default="alias",
    )
    ...
    parser.add_argument("--batch-belief-tuple-tries", type=int, default=3)
    parser.add_argument("--batch-belief-hier-cdf-block", type=int, default=32)
    parser.add_argument("--batch-belief-cdf-build-warps", type=int, default=8)
    parser.add_argument("--batch-belief-cdf-sample-warps", type=int, default=4)
