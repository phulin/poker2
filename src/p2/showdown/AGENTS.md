...
- `compare_multiway_showdown_tiers.py`: Copied tier-comparison benchmark implementation with package-relative imports, tier 1-4 by-hand outputs, cached board/rank prefix context, vectorized rank-prefix Triton factor kernels for tier 2, tier 3 reuse of rank-prefix factors, and a memory-aware streaming Triton wedge path for large four-player CUDA batches.
