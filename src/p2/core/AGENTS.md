...
- `structured_config.py`: Dataclass-based Hydra config schema and enums for training, model, environment, search, data-source, curriculum, preflop validation, pregeneration, and rating options, including per-curriculum-substep model overrides, actor-backup consistency loss knobs, and native 169 preflop all-in oracle paths.
