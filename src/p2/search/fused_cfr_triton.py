...
if triton is not None:
    ...
    def _preflop_sample_snapshot_multiway_kernel(
        ...
        hand_mask = hands < H
        t = tl.load(t_ptr)
        sample_row = tl.load(t_sample_ptr + rows, mask=row_mask, other=-1) == t
        mask = sample_row[:, None] & hand_mask[None, :]

        policy_vals = tl.load(
            policy_ptr + rows[:, None] * H + hands[None, :],
            mask=mask,
            other=0.0,
        )
        tl.store(
            policy_sample_ptr + rows[:, None] * H + hands[None, :],
            policy_vals,
            mask=mask,
        )
        for player in tl.static_range(0, NUM_PLAYERS):
            belief_vals = tl.load(
                beliefs_ptr
                + (rows[:, None] * NUM_PLAYERS + player) * H
                + hands[None, :],
                mask=mask,
                other=0.0,
            )
...
def fused_preflop_sample_snapshot_multiway_(
    policy: torch.Tensor,
    ...
    """Copy current policy/belief rows whose sampled CFR iteration equals ``t``."""
    if not triton_is_available():
        raise RuntimeError("Triton is not installed.")
    assert policy.is_contiguous() and policy.dim() == 2
    ...
    assert t_sample.is_contiguous() and t_sample.shape == (total,)
    assert t_tensor.is_cuda and t_tensor.dim() == 0
    if h > block_h:
        raise ValueError(f"hand dim {h} exceeds block_h {block_h}")
    _preflop_sample_snapshot_multiway_kernel[
        (triton.cdiv(total, block_m), triton.cdiv(h, block_h))
    ](
        policy,
        policy_sample,
        beliefs,
...
if triton is not None:
    ...
    @triton.jit
    def _fused_hu_closing_postprocess_writeback_multiway_kernel(
        h_ptr,  # [M, 2, H] heads-up closing hand_values
        l_ptr,  # [M, P, H] previous postprocessed last values
        b_ptr,  # [M, P, H] beliefs
        idx_ptr,  # [M] absolute latest/env row
        live_players_ptr,  # [M, 2]
        latest_ptr,  # [T, P, H]
        last_out_ptr,  # [M, P, H]
        folded_ptr,  # [T, P] bool
        stacks_ptr,  # [T, P]
        ...
        live0 = tl.load(live_players_ptr + m * 2)
        live1 = tl.load(live_players_ptr + m * 2 + 1)
        is0 = players == live0
        is1 = players == live1
        closing0 = tl.load(
            h_ptr + (m * 2) * H + offs[None, :],
            mask=hand_mask[None, :],
            other=0.0,
        ).to(tl.float32)
        closing1 = tl.load(
            h_ptr + (m * 2 + 1) * H + offs[None, :],
            mask=hand_mask[None, :],
            other=0.0,
        ).to(tl.float32)
        ...
        live = ~folded
        live_mask = live[:, None] & mask
        raw_values = tl.where(is0[:, None], closing0, stack_value[:, None])
        raw_values = tl.where(is1[:, None], closing1, raw_values)
        values = tl.where(live[:, None], raw_values, stack_value[:, None])
...
def fused_hu_closing_postprocess_writeback_multiway_(
    hand_values: torch.Tensor,  # [M, 2, H]
    last_model_values: torch.Tensor,  # [M, P, H]
    beliefs: torch.Tensor,  # [M, P, H]
    node_indices: torch.Tensor,  # [M]
    live_players: torch.Tensor,  # [M, 2]
    latest_values: torch.Tensor,  # [T, P, H]
    last_out: torch.Tensor,  # [M, P, H]
    has_folded: torch.Tensor,  # [T, P]
    stacks: torch.Tensor,  # [T, P]
    ...
    """Postprocess heads-up closing values and write full multiway rows."""
    if not triton_is_available():
        raise RuntimeError("Triton is not installed.")
    assert hand_values.is_contiguous()
    assert last_model_values.is_contiguous()
    assert beliefs.is_contiguous()
    assert node_indices.is_contiguous()
    assert live_players.is_contiguous()
    assert latest_values.is_contiguous()
    assert last_out.is_contiguous()
    assert has_folded.is_contiguous()
    assert stacks.is_contiguous()
    ...
    assert scale.is_contiguous()
    assert hand_values.dim() == 3 and hand_values.shape[1] == 2
    assert last_model_values.shape == beliefs.shape == last_out.shape
    m, players, h = last_model_values.shape
    assert hand_values.shape == (m, 2, h)
    assert live_players.shape == (m, 2)
    assert node_indices.shape == (m,)
    assert latest_values.shape[1:] == (players, h)
    assert has_folded.shape == (latest_values.shape[0], players)
    assert stacks.shape == starting_stacks.shape == (latest_values.shape[0], players)
    ...
    while block_p < players:
        block_p *= 2
    _fused_hu_closing_postprocess_writeback_multiway_kernel[(m,)](
        hand_values,
        last_model_values,
        beliefs,
        node_indices,
...
if triton is not None:
    ...
    @triton.jit
    def _fused_hu_closing_selected_beliefs_writeback_multiway_kernel(
        h_ptr,  # [M, 2, H] heads-up closing hand_values
        l_ptr,  # [M, P, H] previous postprocessed last values
        b2_ptr,  # [M, 2, H] selected live-player beliefs
        idx_ptr,  # [M] absolute latest/env row
        live_players_ptr,  # [M, 2]
        latest_ptr,  # [T, P, H]
        last_out_ptr,  # [M, P, H]
        ...
        folded = tl.load(folded_ptr + env_player_ptrs, mask=player_mask, other=1)
        live = ~folded
        live_count = tl.sum(tl.where(live & player_mask, 1.0, 0.0))
        scale = tl.load(scale_ptr + row).to(tl.float32)
        scale = tl.maximum(scale, 1.0)
        stack_value = (
            tl.load(stacks_ptr + env_player_ptrs, mask=player_mask, other=0.0).to(
        ...
        selected_belief_sum = tl.sum(belief0 + belief1)
        selected_expected = tl.sum(closing0 * belief0 + closing1 * belief1)
        nonselected = ~(is0 | is1) & player_mask
        nonselected_expected = tl.sum(tl.where(nonselected, stack_value, 0.0))
        expected_sum = selected_expected + nonselected_expected
        # Belief rows are normalized during propagation; selected sums are read
        # from the actual selected beliefs to preserve tiny numerical drift.
        denom = selected_belief_sum + tl.maximum(live_count - 2.0, 0.0)
        correction = expected_sum / tl.maximum(denom, 1.0e-12)

        raw_values = tl.where(is0[:, None], closing0[None, :], stack_value[:, None])
        raw_values = tl.where(is1[:, None], closing1[None, :], raw_values)
        processed = tl.where(live[:, None], raw_values - correction, raw_values)
...
def fused_hu_closing_selected_beliefs_writeback_multiway_(
    hand_values: torch.Tensor,  # [M, 2, H]
    last_model_values: torch.Tensor,  # [M, P, H]
    selected_beliefs: torch.Tensor,  # [M, 2, H]
    node_indices: torch.Tensor,  # [M]
    live_players: torch.Tensor,  # [M, 2]
    latest_values: torch.Tensor,  # [T, P, H]
    last_out: torch.Tensor,  # [M, P, H]
    ...
    """Postprocess HU closing values using selected live-player beliefs only."""
    if not triton_is_available():
        raise RuntimeError("Triton is not installed.")
    assert hand_values.is_contiguous()
    assert last_model_values.is_contiguous()
    assert selected_beliefs.is_contiguous()
    assert node_indices.is_contiguous()
    assert live_players.is_contiguous()
    assert latest_values.is_contiguous()
    assert last_out.is_contiguous()
    ...
    assert scale.is_contiguous()
    assert hand_values.dim() == 3 and hand_values.shape[1] == 2
    assert last_model_values.shape == last_out.shape
    m, players, h = last_model_values.shape
    assert hand_values.shape == (m, 2, h)
    assert selected_beliefs.shape == (m, 2, h)
    assert live_players.shape == (m, 2)
    assert node_indices.shape == (m,)
    assert latest_values.shape[1:] == (players, h)
    assert has_folded.shape == (latest_values.shape[0], players)
    ...
    while block_p < players:
        block_p *= 2
    _fused_hu_closing_selected_beliefs_writeback_multiway_kernel[(m,)](
        hand_values,
        last_model_values,
        selected_beliefs,
        node_indices,
