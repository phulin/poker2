...
- `preflop_sparse_cfr_evaluator.py`: Guarded non-fused sparse CFR boundary for the `S_preflop`/`S_0` arbitrary preflop public-state model, requiring PBSEnv street-0 roots, supporting compact 169-hand tensors with exact class-level blocker projection for class-constant strategies, and generation-time continuation-depth value sampling.
