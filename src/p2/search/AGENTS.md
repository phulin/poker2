...
- `rebel_solved_dataset.py`: Tensor-only bounded solved-example dataset writer/reader for postflop value and policy `RebelBatch` shards with manifest validation, wrapped sampling, optional compressed float storage, and optional pinned/async shard prefetch.
