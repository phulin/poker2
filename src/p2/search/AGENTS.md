...
- `preflop_sparse_cfr_evaluator.py`: Guarded compact-only non-fused sparse CFR boundary for the `S_preflop`/`S_0` arbitrary preflop public-state model, requiring PBSEnv street-0 roots, 169-hand policy/value models, exact class-level blocker projection for class-constant strategies, generation-time continuation-depth value sampling, and native 169 preflop all-in call leaf routing by live-player count.
- `fused_preflop_sparse_cfr_evaluator.py`: CUDA/Triton-backed compact-only preflop sparse evaluator boundary that uses 169-hand sparse tree tensors, exact class-level blocker projection for EV/regret math, native 169 preflop all-in call values, and generic fused average-policy/value/model-writeback kernels that are parameterized by hand dimension.
