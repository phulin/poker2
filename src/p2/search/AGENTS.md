...
### Source files
- `cfr_evaluator.py`: PublicBeliefState, exploitability stats, hand-rank data, evaluator interface, shared split policy/value model dispatch, optional closing-leaf value model routing, hand-dimension-aware sparse helpers, sparse warm-start prior/regret extraction helpers, sparse-only delayed DCFR-hybrid PCFR/SAPCFR predictive regret extraction, and per-root leaf target-source provenance.
...
- `postflop_spot_sampler.py`: Random legal postflop public-root samplers for heads-up flop/turn/river street-start and legal-prefix roots, closed-street chance-target roots with randomized pre-chance beliefs, native compact 169-hand preflop chance-target beliefs, texture-stratified boards, tensorized strength-ordered and named-shape board-legal belief mixtures, and conservative pot/SPR templates.
...
- `preflop_sparse_cfr_evaluator.py`: Guarded non-fused sparse CFR boundary for the `S_preflop`/`S_0` arbitrary preflop public-state model, requiring PBSEnv street-0 roots, supporting compact 169-hand tensors with no-blocker class-level backups/regrets, and generation-time continuation-depth value sampling.
- `fused_preflop_sparse_cfr_evaluator.py`: CUDA/Triton-backed 1326-combo preflop sparse evaluator boundary with multiway fused EV/regret, average-policy/value update, and model-writeback kernels plus reference PBSEnv construction; compact 169 preflop currently fails early and should use the non-fused evaluator.
