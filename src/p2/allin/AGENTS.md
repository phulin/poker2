...
- `train.py`: Hydra-driven standalone training script with a separate Weights & Biases project, Muon/AdamW split optimization for linear matrices vs. scalar parameters, optional cosine/linear LR decay and stable warmdown schedules, opt-in torch.compile support, checkpoint resume support, batch-size phase scheduling, online target workspace reuse, multi-epoch pregenerated train/validation data support with pinned CUDA prefetch, epoch-aware suit and player permutation augmentation, and detailed throughput/loss/target/eval logging including validation MSE/MAE by live player count.
