...
- `train.py`: Hydra-driven standalone training script with a separate Weights & Biases project, Muon/AdamW split optimization for linear matrices vs. scalar parameters, optional cosine LR decay, opt-in torch.compile support, checkpoint resume support, batch-size phase scheduling, multi-epoch pregenerated train/validation data support with pinned CUDA prefetch, epoch-aware suit and player permutation augmentation, and detailed throughput/loss/target logging.
