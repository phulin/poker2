...
from p2.env.rules_triton import compare_7_single_batch_triton, triton_is_available
...
def make_random_batch(batch_size: int, device: torch.device, seed: int) -> torch.Tensor:
...
@torch.no_grad()
def verify_correctness(ab_batch: torch.Tensor) -> None:
...
def main() -> None:
    ...
    for batch_size in [int(x) for x in args.batch_sizes.split(",") if x.strip()]:
        ab_batch = make_random_batch(batch_size, device, args.seed + batch_size)
        verify_correctness(ab_batch[: min(batch_size, 4096)])
