#!/usr/bin/env python3
"""Microbenchmark HU closing writeback using selected live-player beliefs."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("/tmp/preflop_selected_hu_writeback.json"),
    )
...
def main() -> None:
    ...
    with _pause_processes(not args.no_pause, args.pause_pattern):
        ...
        with torch.no_grad():
            ...
            for flag in ("0", "1") * max(1, args.warmup_iters):
                os.environ["P2_PREFLOP_SELECTED_HU_CLOSING_BELIEFS"] = flag
                ev._set_model_values(1, beliefs, features)
            _sync(ev.device)

            os.environ["P2_PREFLOP_SELECTED_HU_CLOSING_BELIEFS"] = "0"
            ev._set_model_values(1, beliefs, features)
            reference = ev.latest_values.clone()
            os.environ["P2_PREFLOP_SELECTED_HU_CLOSING_BELIEFS"] = "1"
            ev._set_model_values(1, beliefs, features)
    ...
    for row in rows_out:
        print(f"selected_hu={row['selected_hu']} ms={row['ms']:.6f}", flush=True)
