...
- `bench_preflop_belief_projection.py`: CUDA microbenchmark for compact preflop token-input belief projections, comparing separate range/bucket matmuls with a fused range-plus-bucket projection and dynamic compiled variants.
- `bench_preflop_eval_projection_cache.py`: CUDA microbenchmark for the compact preflop static value forward, comparing a cold dynamic-compiled trace that recomputes hand/range projection work with a trace pre-warmed to use autocast-aware eval projection cache buffers.
