...
fn hand_card0(hand: u32) -> u32 {
  var remaining = hand;
  for (var c0 = 0u; c0 < 51u; c0 = c0 + 1u) {
    let count = 51u - c0;
    if (remaining < count) {
      return c0;
    }
...
fn hand_card1(hand: u32) -> u32 {
  var remaining = hand;
  for (var c0 = 0u; c0 < 51u; c0 = c0 + 1u) {
    let count = 51u - c0;
    if (remaining < count) {
      return c0 + 1u + remaining;
    }
    remaining = remaining - count;
  }
  return 51u;
...
@group(0) @binding(4) var<storage, read_write> childValues: array<f32>;
@group(0) @binding(5) var<uniform> params: Params;
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  let c0 = hand_card0(hand);
  let c1 = hand_card1(hand);
  if (overlaps_board(base, &childStates, c0, c1)) {
    childValues[out] = 0.0;
    return;
  }
  let b0 = as_u(childStates[base + BOARD0]);
  let b1 = as_u(childStates[base + BOARD0 + 1u]);
  let b2 = as_u(childStates[base + BOARD0 + 2u]);
  let b3 = as_u(childStates[base + BOARD0 + 3u]);
  let b4 = as_u(childStates[base + BOARD0 + 4u]);
  let rank = evaluate7(c0, c1, b0, b1, b2, b3, b4);
  ...
  for (var opp = 0u; opp < NUM_HANDS; opp = opp + 1u) {
    let o0 = hand_card0(opp);
    let o1 = hand_card1(opp);
    if (overlaps(c0, c1, o0, o1) || overlaps_board(base, &childStates, o0, o1)) {
      continue;
    }
    let oppRank = evaluate7(o0, o1, b0, b1, b2, b3, b4);
    if (player == 0u) {
