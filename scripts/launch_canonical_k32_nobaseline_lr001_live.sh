...
# Live run: equity-input canonical k32 head with NO analytic additive baseline
# (the canonical head's built-in per-bin equity features are the only equity
# signal; value is learned from scratch rather than as a residual over the
# river-range-equity baseline). Same live architecture as the r96-canonical
# baseline run, but the value_river_range_equity_* baseline flags are removed
# and LR is lowered to 0.01.
...
LR=${LR:-0.01}
LR_FINAL=${LR_FINAL:-0.001}
LR_TAG=${LR_TAG:-lr001}
CHECKPOINT_DIR=${CHECKPOINT_DIR:-checkpoints-rebel-curriculum-sapcfr-80-40-300it-8000-val-ctx41-live-board96-belief128-canonical-k32-nobaseline-out0-${LR_TAG}-random-wandb}
WANDB_NAME=${WANDB_NAME:-river_sapcfr_80_40_300it_8000_val_ctx41_live_board96_belief128_canonical_k32_nobaseline_out0_${LR_TAG}_random}
...
exec uv run python -m p2.cli.train_rebel_curriculum \
  ...
  ++model.value_river_canonical_head=true \
  ++model.value_river_canonical_bins=32 \
  ++model.value_river_canonical_dim=64 \
  ++model.value_river_canonical_layers=2 \
