...
- `distill_epreflop_6p_live_pair.py`: Trains a value-only 6-player compact `E_preflop` model from preflop street-closed state shards by sampling random compact beliefs and averaging frozen 2-player `E_preflop` targets over all uniform live-player pairs.
