...
- `bench_preflop_evaluate_cfr_loop.py`: Benchmarks the production compact preflop `evaluate_cfr` solve loop on the current actions_4_7 shape with optional preflop backward-induction process-group pause/resume handling.
- `analyze_active_preflop_open_grids.py`: Loads the active 6-player preflop specialist checkpoint, uses it as the mixed-street cutoff model until the fold-through state reaches four preflop actions, switches SB-and-later solving to the checkpoint config's original cutoff model, runs compact preflop CFR for LJ then fold-through HJ/CO/BTN/SB/BB open nodes, prints call/fold/bet/all-in 169-class range grids, and saves per-position evaluator tree snapshots for inspection.
