...
- `preflop_backward_induction.py`: Trains depth-4 preflop backward-induction bucket specialists from pregenerated public states with random compact beliefs and fixed CFR solver checkpoints, optionally writes solved `RebelBatch` shards, logs to W&B, and includes a distillation subcommand that trains one model from the four bucket specialists while omitting 0-3 value supervision.
