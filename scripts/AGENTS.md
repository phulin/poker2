...
- `bench_cfr_main_path.py`: Runs realistic ReBeL CFR train-step source profiling, full-solve profiling, data-generation profiling, and component microbenchmarks with optional train_rebel pause/resume handling and configurable starting CFR iteration for post-delay kernel timing.
...
- `bench_preflop_full_loop_profile.py`: Profiles eager 6-player compact preflop CFR iterations on packed bucket states from the actions_4_7 training shape, including component and kernel timing, while pausing matching preflop backward-induction process groups.
- `bench_preflop_evaluate_cfr_loop.py`: Benchmarks the production compact preflop `evaluate_cfr` solve loop on the current actions_4_7 shape, including optional segmented model-eval reporting, multi-tree sampling, graph/parallel-eval toggles, and preflop backward-induction process-group pause/resume handling.
- `analyze_active_preflop_open_grids.py`: Loads the active 6-player preflop specialist checkpoint as the same-street cutoff model, uses the checkpoint config's original end-of-street checkpoint for new-street closing leaves, runs compact preflop CFR for LJ then fold-through HJ/CO/BTN/SB/BB open nodes, prints call/fold/bet/all-in 169-class range grids, and saves per-position evaluator tree snapshots for inspection.
- `bench_preflop_fused_hotloop.py`: Microbenchmarks compact preflop fused hot-loop projection, source-weight, and EV-backup kernels versus legacy tensor expressions, pausing live `train_rebel` processes with SIGSTOP/SIGCONT while timing.
...
- `extract_preflop_street_closed_states.py`: Scans saved compact 6-player preflop public-state buckets, steps every legal action that closes the preflop street with force-HU closure disabled, and writes the resulting street-closed states plus source/action metadata for live-pair E_preflop distillation.
- `distill_epreflop_6p_live_pair.py`: Trains a value-only 6-player compact `E_preflop` model from preflop street-closed state shards by sampling configured compact belief profiles and averaging frozen 2-player `E_preflop` targets over all uniform live-player pairs.
- `probe_eos_in_domain_beliefs.py`: Reconstructs saved BTN end-of-street leaf states, samples uniform or random training-style compact beliefs, compares direct 6-player EOS student values against the frozen 2-player `E_preflop` live-pair teacher, and writes raw/postprocessed error summaries.
- `sample_preflop_continuation_beliefs.py`: Solves packed `actions_0_3` preflop roots through the current 4-7, 8-11, and 12-end specialist chain, sampling configured root beliefs plus continuation PBSs between buckets and saving propagated root beliefs/env states for distribution analysis.
- `sweep_preflop_value_lr_bs.py`: Trains one value-only epoch over a pregenerated preflop solved dataset for each learning-rate/batch-size/schedule/optimizer variant, evaluates against a fixed validation cache, writes per-trial JSON results, and saves the best checkpoint.
- `preflop_8_11_pregen_lr_sweep.py`: Pregenerates fixed actions_8_11 solved value/policy shards from the production cutoff setup, then runs W&B-logged supervised LR sweeps over 100/200-step windows from a shared specialist checkpoint without re-solving CFR per trial.
