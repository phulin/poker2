...
- `preflop_backward_induction.py`: Trains depth-4 preflop backward-induction bucket specialists from packed pregenerated public-state buckets with DataLoader shuffling, optional per-bucket CFR evaluator batch sizes, random compact beliefs, fixed CFR solver checkpoints, cached high-iteration validation sets, optional solved `RebelBatch` shard writes, W&B logging, and a distillation subcommand that trains one model from the four bucket specialists while omitting 0-3 value supervision.
