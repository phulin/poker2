...
# Resumes wandb run ool717bi (poker-rebel-cfr, "hu-context-v2").
# The process died after writing checkpoints-rebel-hu-context-v2/rebel_latest.pt
# at internal step 7149 (displayed step 7150) on 2026-07-21 04:39. All overrides
# below are copied verbatim from that run's launch (see
# launch_rebel_10k_from_o6vbd6ax.sh) plus its checkpoint_dir; resume_from
# restores model, optimizer, RNG, and replay buffers and continues the same
# wandb run to num_steps=10000.
uv run python -m p2.cli.train_rebel \
  ...
  checkpoint_dir=checkpoints-rebel-hu-context-v2 \
  +resume_from=checkpoints-rebel-hu-context-v2/rebel_latest.pt
