...
def test_calculate_unblocked_mass_uniform_beliefs() -> None:
    """Test _calculate_unblocked_mass with uniform beliefs."""
    ...
    uniform_beliefs = torch.full(
        (1, NUM_HANDS), 1.0 / NUM_HANDS, dtype=evaluator.float_dtype
    )

    # Compute unblocked mass
    unblocked = evaluator._calculate_unblocked_mass(uniform_beliefs)
...
def test_calculate_unblocked_mass_delta_beliefs() -> None:
    """Test _calculate_unblocked_mass with delta beliefs (single hand has all mass)."""
    evaluator, _ = make_evaluator(batch_size=1, max_depth=0)

    # Delta beliefs: one hand has all the mass
    delta_beliefs = torch.zeros(1, NUM_HANDS, dtype=evaluator.float_dtype)
    target_hand = 100  # Pick an arbitrary hand
    delta_beliefs[0, target_hand] = 1.0

    # Compute actual result
    unblocked = evaluator._calculate_unblocked_mass(delta_beliefs)
    ...
    combo_onehot = combo_to_onehot_tensor(device=evaluator.device).float()
    blocking_matrix = (
...
def test_calculate_unblocked_mass_double_half_delta() -> None:
    """Test _calculate_unblocked_mass with double-half-delta beliefs (two hands each have half mass)."""
    evaluator, _ = make_evaluator(batch_size=1, max_depth=0)

    # Double-half-delta: two hands each have 0.5 mass
    double_half_delta = torch.zeros(1, NUM_HANDS, dtype=evaluator.float_dtype)
    ...
    # Compute actual result
    unblocked = evaluator._calculate_unblocked_mass(double_half_delta)

    # Hardcoded expectations:
    # Each hand blocks itself, so: unblocked = 1.0 - 1.0 + 0.5 = 0.5
    assert torch.allclose(unblocked[0, hand1], torch.tensor(0.5), atol=1e-6, rtol=1e-6)
    assert torch.allclose(unblocked[0, hand2], torch.tensor(0.5), atol=1e-6, rtol=1e-6)
...
def test_calculate_unblocked_mass_matches_compatibility_matrix() -> None:
    ...
    test_cases = [
        # Uniform beliefs
        torch.full((1, NUM_HANDS), 1.0 / NUM_HANDS, dtype=evaluator.float_dtype),
        # Delta beliefs
        torch.zeros(1, NUM_HANDS, dtype=evaluator.float_dtype),
        # Double-half-delta
        torch.zeros(1, NUM_HANDS, dtype=evaluator.float_dtype),
    ]
    ...
    eye = torch.eye(NUM_HANDS, device=evaluator.device, dtype=evaluator.float_dtype)
    ones = torch.ones(
        NUM_HANDS, NUM_HANDS, device=evaluator.device, dtype=evaluator.float_dtype
    )
    compatible_matrix = ones - blocking_matrix + eye  # [NUM_HANDS, NUM_HANDS]

    for target in test_cases:
        # Optimized version (current implementation)
        unblocked_optimized = evaluator._calculate_unblocked_mass(target)
