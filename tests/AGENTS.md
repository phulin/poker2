...
- `test_rebel_solved_dataset.py`: Tensor-only solved ReBeL dataset serialization, manifest validation and street/depth/target-source/root-source coverage counts, wrapped reads, random sampling, compressed float storage, and async prefetch coverage.
...
- `test_train_rebel_curriculum.py`: Curriculum CLI orchestration coverage for train/distill substep routing, per-substep checkpoint dirs, promotion state, metadata, and validated substep-aware resume.
