...
- `test_postflop_spot_sampler.py`: Random postflop public-root sampler invariants for flop/turn/river street-start and legal-prefix roots, closed-street chance-target roots with randomized pre-chance beliefs, native compact preflop beliefs, board texture stratification, randomized board-legal beliefs, and conservative pot/SPR templates.
...
- `test_train_rebel_curriculum.py`: Curriculum stage orchestration coverage for train/distill substep routing, per-substep checkpoint dirs, promotion state, metadata, S_i policy warm-starts, value-only E_i ownership, and validated substep-aware resume.
