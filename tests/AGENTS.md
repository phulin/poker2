...
- `test_rebel_solved_dataset.py`: Tensor-only solved ReBeL dataset serialization, manifest validation and street/depth/target-source coverage counts, wrapped reads, random sampling, compressed float storage, and async prefetch coverage.
- `test_pregenerate_postflop_rebel.py`: Bounded postflop pregeneration CLI coverage for live-mode validation, trimmed solved-batch writer calls, root-street metadata, feature-encoder metadata, quality metadata, and closing-leaf/distillation-source checkpoint provenance.
...
- `test_rebel_loop.py`: Shared ReBeL loop runner control-flow coverage for checkpoints, stats, final save, and TrueSkill snapshots without invoking real CFR.
