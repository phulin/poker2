...
class TestLegalActions:
    ...
    def test_postflop_betting_actions(self):
        ...
        for expected in expected_sizes:
            assert expected in actual_sizes, f"Expected bet size {expected} not found in {actual_sizes}"
    ...
    def test_facing_bet_actions(self):
        ...
        state = env.state
        assert state.players[1].committed == 40  # Player 1 (big blind) committed the bet
        assert state.players[0].committed == 0   # Player 0 (small blind) hasn't acted yet
        assert state.pot == 80
    ...
    def test_raise_sizes_after_bet(self):
        ...
        for action in raise_actions:
            assert action.amount > 40, f"Raise amount {action.amount} should be > call amount 40"
...
class TestQuantizedRaiseSizes:
    ...
    def test_preflop_raise_sizes(self):
        ...
        assert raise_amounts == expected_raises, f"Expected {expected_raises}, got {raise_amounts}"
    ...
    def test_postflop_bet_sizes(self):
        ...
        assert bet_amounts == expected_bets, f"Expected {expected_bets}, got {bet_amounts}"
    ...
    def test_zero_blinds_scenario(self):
        ...
        assert len(bet_actions) == 0, f"Should not offer bet actions when total_committed is 0, got {bet_actions}"
    ...
    def test_raise_sizes_consistency(self):
        ...
        for sb, bb, expected_total, expected_call, expected_raises in scenarios:
            ...
            total_committed = state.pot + state.players[0].committed + state.players[1].committed
            assert total_committed == expected_total, f"Expected total_committed {expected_total}, got {total_committed}"
            
            # Verify call amount
            to_call = state.players[1].committed - state.players[0].committed
            assert to_call == expected_call, f"Expected call amount {expected_call}, got {to_call}"
            ...
            assert raise_amounts == expected_raises, f"Expected raises {expected_raises}, got {raise_amounts}"
